...
def get_babi_raw(id, test_id):
    ...
    babi_train_raw = init_babi(os.path.join(os.path.dirname(
        os.path.realpath(__file__)), 'data/en-10k/%s_train.txt' % babi_name))
    babi_test_raw = init_babi(os.path.join(os.path.dirname(
        os.path.realpath(__file__)), 'data/en-10k/%s_test.txt' % babi_test_name))
...
def load_babi(split_sentences=False):
    ...
    babi_train_raw, babi_test_raw = get_babi_raw(
        FLAGS.babi_id, FLAGS.babi_test_id)
    ...
    train_data = process_input(
        babi_train_raw, np.float32, word2vec, vocab, ivocab, FLAGS.embed_size, split_sentences)
    print('vocab length after train inputs:', len(vocab))
    print('==> get test inputs')
    test_data = process_input(babi_test_raw, np.float32, word2vec,
                              vocab, ivocab, FLAGS.embed_size, split_sentences)
    ...
    if FLAGS.word2vec_init:
        ...
        word_embedding = np.random.uniform(-FLAGS.embedding_init,
                                           FLAGS.embedding_init, (len(ivocab), FLAGS.embed_size))
    ...
    for iter_ in range(loopcount):
        ...
        max_input_len = max(
            min(np.max(input_lens), FLAGS.max_allowed_inputs), max_input_len)
        ...
        if split_sentences:
            ...
            inputs = pad_inputs(inputs, input_lens, max_input_len)
            input_masks = pad_inputs(
                input_masks, mask_lens, max_mask_len, "mask")

        questions = pad_inputs(questions, q_lens, max_q_len)

        answers = np.squeeze(np.stack(
            answers)) if FLAGS.babi_id == 19 else np.expand_dims(np.stack(answers), 1)
        ...
        if iter_ == 0:
            train = questions[:FLAGS.num_train], inputs[:FLAGS.num_train], q_lens[:FLAGS.num_train], input_lens[:
                                                                                                                FLAGS.num_train], input_masks[:FLAGS.num_train], answers[:FLAGS.num_train], rel_labels[:FLAGS.num_train]

            valid = questions[FLAGS.num_train:], inputs[FLAGS.num_train:], q_lens[FLAGS.num_train:], input_lens[
                FLAGS.num_train:], input_masks[FLAGS.num_train:], answers[FLAGS.num_train:], rel_labels[FLAGS.num_train:]
        else:
            test = questions, inputs, q_lens, input_lens, input_masks, answers, rel_labels

    data = [train, valid, test] if (FLAGS.mode == 'train') else [
        train, valid, test]
